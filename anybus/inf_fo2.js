...
function WriteChannelInfoForChannel(InfoArray)
{
  ...
  var SignalStrength = InfoArray[6];
  ...
  SetInnerHtmlValue("Redundancy"+ChannelNum,GetOnOffText(Redundancy));
  SetInnerHtmlValue("Setting"+ChannelNum,GetSwDswText(Setting));
  SetInnerHtmlValue("SignalStrength"+ChannelNum,SignalStrength);
...
function PeriodicTimerUpdateStatusData()
{
  ...
  if (CardSlot >= 0){
    var DataParams     = "data=fiber2-info:"+CardSlot;
...
function DynamicCreateTable(ImageIdx)
{
  ...
  var RightIds = new Array("Network"+ImageIdx,"Baudrate"+ImageIdx,"StationCount"+ImageIdx,"Redundancy"+ImageIdx,"Setting"+ImageIdx,"SignalStrength"+ImageIdx);
  var LeftIds = new Array("네트워크:","전송속도:","채널에서 스테이션 카운트 활성화:","이중화:","설정:","신호 세기:");
