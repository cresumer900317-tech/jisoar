  ...
  if (xmlhttpStatusData != null){
    if (xmlhttpStatusData.readyState == 4){
      if (xmlhttpStatusData.status == 200){
        ...
        GenericModuleInfo[12] = WriteChannelInfo(Inputs ,"Input" );
...
function WriteModuleInfo(InfoArray)
{
  ...
  if (InfoArray.length >= 8){
    ...
    NumberOfOutCh= InfoArray[11];
    NumberOfInCh = InfoArray[12];
  ...
  SetInnerHtmlValue("outchannelcount",NumberOfOutCh);
  SetInnerHtmlValue("inchannelcount",NumberOfInCh);
}
