...
function WriteChannelInfoForChannel(InfoArray,VersionArray)
{
  ...
  if (InfoArray.length < 10) return false;
  ...
  var DCVoltage    = InfoArray[4];
  var DCVplus      = InfoArray[5];
  var DCVmin       = InfoArray[6];
  var DCVnoise     = InfoArray[7];
  var DCVunbal     = InfoArray[8];
  var CurrentCons  = InfoArray[9];
  ...
  if ((ClusterNum & 0x07) == 0x07){
    ...
    SetInnerHtmlValue("Network"+ChannelNum,(ClusterNum+1) + " (" + ClusterName + ")" + " "+ "(Only events are registered on this network, the module has no repeating functionality)");
  }
  */
...
function PeriodicTimerUpdateStatusData()
{
  ...
  if (CardSlot >= 0){
    ...
    var DataParams     = "data=ffmonitor-info:"+CardSlot;
...
function DynamicCreateTable(ImageIdx)
{
  ...
  var RightIds = new Array("StationCount"+ImageIdx,"DcVolt"+ImageIdx,"DcPlus"+ImageIdx,"DcMin"+ImageIdx,"DcNoise"+ImageIdx,"DcUnbal"+ImageIdx,"Current"+ImageIdx);
  var LeftIds = new Array("채널에서 스테이션 카운트 활성화:","DC 전압:","DC 플러스:","DC 최소:","DC 노이즈:","DC 불균형:","전류 소비량:");
